...
def get_hourly_trend(stats, num_hours):
    ...
    #
    start = datetime.fromisoformat(stats[0]["datetime"].replace('Z', '+00:00')).timestamp()
    #end = datetime.fromisoformat(stats[-1]["datetime"].replace('Z', '+00:00')).timestamp() # Debugging
    #print(f"Debug num_hours: {num_hours}, start: {stats[0]['datetime']}, end: {stats[-1]['datetime']}, diff: {(start - end)}")
    ...
    #
    for row in stats:
        time_t = datetime.fromisoformat(row["datetime"].replace('Z', '+00:00')).timestamp()

        if time_t <= target:
            #print(f"Target found: {row['datetime']}") # Debugging
